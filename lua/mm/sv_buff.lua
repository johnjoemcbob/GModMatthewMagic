...
function plymeta:AddBuff( id )
	...
	if ( not self:GetBuff( id ) ) then
		MM_Buffs[id]:Init( self )
	end

	-- Flag as affecting this player
	self.Buffs[id] = CurTime() + MM_Buffs[id].Time
...
hook.Add( "PlayerInitialSpawn", "PlayerInitialSpawn_Buff", function( ply )
	ply.Buffs = {}
end )

hook.Add( "Think", "Think_Buff", function()
	for k, ply in pairs( player.GetAll() ) do
		-- Run logic on every player and every buff to find if the buff should be added to that player
		for m, buff in pairs( MM_Buffs ) do
