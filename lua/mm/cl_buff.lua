...
hook.Add( "InitPostEntity", "InitPostEntity_Buffs", function()
	-- Create a material cache for each icon
	for k, v in pairs( MM_Buffs ) do
		v.Material = Material( v.Icon )
	end
end )

hook.Add( "HUDPaint", "HUDPaint_Buffs", function()
	local x = 0
	local y = ScrH() / 4
	local size = ScrH() / 20
	for k, v in pairs( Buffs ) do
		if ( v and ( v > 0 ) ) then
			local activetime = v - CurTime()
				...
				if ( MM_Buffs[k].Time == 0.5 ) then
					activetime = MM_Buffs[k].Time
				end
			if ( activetime > 0 ) then
				...
				surface.SetDrawColor( 0, 255, 0, 150 )
					if ( MM_Buffs[k].Debuff ) then
						surface.SetDrawColor( 255, 0, 0, 150 )
					end
				surface.DrawRect( x, y, size / MM_Buffs[k].Time * activetime, size )
				...
				if (
					...
					draw.DrawText( MM_Buffs[k].Name, "TargetID", dx, dy, textcolour, TEXT_ALIGN_LEFT )
					...
					draw.DrawText( MM_Buffs[k].Description, "TargetID", dx, dy, textcolour, TEXT_ALIGN_LEFT )
				end
